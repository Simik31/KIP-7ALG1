...
public class BilliardBalls {
    ...
    public void generateAllBalls() {
        String[] colors = {"White", "Yellow", "Blue", "Red", "Purple", "Orange", "Green", "Brown", "Black", "Yellow", "Blue", "Red", "Purple", "Orange", "Green", "Brown"};
        String[] types = {"Solid", "Stripes"};
        for(int i = 0; i < allBalls.length; i++) allBalls[i] = new Ball(colors[(i<9)?i:(i-8)], i, types[(i<9)?0:1]);
    }
    ...
    public Ball[] getBallsWithColor(String requiredColor) {
        ...
        int index = 0;
        for(int i = 0; i < allBalls.length; i++) {
            if(requiredColor.equals(allBalls[i].getColor())) tmp[index++] = allBalls[i];
        }
        return tmp;
    }
    ...
    public Ball[] getBallsWithType(String requiredType) {
        ...
        int index = 0;
        for(int i = 0; i < allBalls.length; i++) {
            if(requiredType.equals(allBalls[i].getType())) tmp[index++] = allBalls[i];
        }
        return tmp;
    }
    ...
    public int getNumberSumOfBalls(String requiredType) {
        int total = 0;
        for(int i = 0; i < allBalls.length; i++) {
            if(requiredType.equals(allBalls[i].getType())) total += allBalls[i].getNumber();
        }
        return total;
    }
